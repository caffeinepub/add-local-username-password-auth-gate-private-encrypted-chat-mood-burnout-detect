...
import { Bell, FileText, Inbox, Loader2, MessageSquare } from "lucide-react";
...
export default function TherapistInboxView() {
  ...
  const { data: messages = [], isLoading: messagesLoading } =
    useGetInboxMessages(identity?.getPrincipal().toString() || "");
  const { data: notifications = [], isLoading: notificationsLoading } =
    useGetNotifications(identity?.getPrincipal().toString() || "");
  const { data: sessionNotes = [], isLoading: notesLoading } =
    useGetAllSessionNotes();
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 space-y-2">
                          <div className="flex items-center gap-2">
                            <Bell
                              className={`w-4 h-4 ${notification.read ? "text-muted-foreground" : "text-accent"}`}
                            />
