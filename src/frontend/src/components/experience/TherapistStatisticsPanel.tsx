...
import { Activity, BarChart3, Clock, Loader2, User } from "lucide-react";
...
export default function TherapistStatisticsPanel() {
  ...
  // Filter summaries (all data shown - backend provides aggregate data)
      ...
      <div className="flex items-center gap-2 pb-2 border-b border-border/50">
        ...
        {!isLoading && (
          <Badge variant="secondary" className="ml-auto">
            ...
            {filteredSummaries.length === 1 ? "client" : "clients"}
          </Badge>
        )}
      </div>
        ...
        {error && (
          <div className="flex items-center justify-center h-full">
            <p className="text-destructive text-sm">
              Failed to load client statistics. Please try again.
            </p>
          </div>
        )}
