...
export function classifyMoodAndBurnout(text: string): ClassificationResult {
  ...
  const anxietyMatches = ANXIETY_KEYWORDS.filter((kw) =>
    lowerText.includes(kw),
  ).length;
  const depressionMatches = DEPRESSION_KEYWORDS.filter((kw) =>
    lowerText.includes(kw),
  ).length;
  const stressMatches = STRESS_KEYWORDS.filter((kw) =>
    lowerText.includes(kw),
  ).length;
  const positiveMatches = POSITIVE_KEYWORDS.filter((kw) =>
    lowerText.includes(kw),
  ).length;
  const negativeMatches = NEGATIVE_KEYWORDS.filter((kw) =>
    lowerText.includes(kw),
  ).length;
  ...
  const totalNegativeMatches =
    anxietyMatches + depressionMatches + stressMatches + negativeMatches;

  if (depressionMatches >= 2 || totalNegativeMatches >= 5) {
    burnoutRisk = "high";
  } else if (
    totalNegativeMatches >= 2 ||
    anxietyMatches >= 2 ||
    stressMatches >= 2
  ) {
    burnoutRisk = "medium";
...
export function classifyFromEntry(
  key: string,
  value: string,
