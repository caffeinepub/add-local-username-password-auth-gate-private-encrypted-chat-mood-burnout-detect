...
export default function InboxMessageThread({ messages }: InboxMessageThreadProps) {
    ...
    <div className="p-4 rounded-lg bg-secondary/30 border border-border/30 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-foreground">Thread #{threadId.toString()}</span>
          {hasUnread && (
      ...
      <div className="space-y-3">
        {sortedMessages.map((message) => {
          const isSent = message.sender.toString() !== latestMessage.recipient.toString();
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>{isSent ? 'You' : formatPrincipal(message.sender.toString())}</span>
                </div>
                <div className="flex items-center gap-2">
